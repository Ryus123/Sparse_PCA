...
if __name__ == '__main__': 
    with Pool(processes=n_jobs) as pool:
        for k, M in pool.imap_unordered(monte_carlo_support_error, range(1, 14)):
            results[str(k)] = M
...
plt.plot(liste_k, liste_M, label='F = F1 + lambda * F2')
...
plt.savefig("plot/plot_smallest_m_F1_F2.png")